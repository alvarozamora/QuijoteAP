...
if yt.is_root():
    ...
    nzstd = fiducial_measurements['nzstds']
    ...
    zstds = []
    for i, result in enumerate(storage.values()):
        if i < 15:
            zstds.append(nzstd[i]/np.sqrt(len(result[1])))
    #nzstds = np.array([np.std(result[1]) for result in storage.values()])

    zstds = np.array(zstds)
    ...
    plt.savefig("DirectionalParamScan_numberdensity.png",dpi=230)
    '''
    ...
    plt.savefig("DirectionalParamScan_scale.png",dpi=230)
    '''
    ...
    plt.ylabel("a(z=0.5)")
    plt.title("Quijote Varying Number Density")
    plt.savefig("DirectionalParamScan_numberdensityzOm_p.png",dpi=230)
    '''
    ...
    for dx in np.arange(0.00,0.101,0.01):
        plt.figure(figsize=(10,8))
        #dx = 0.05
        plt.errorbar(ndist+dx, zmeans[ndist], fmt='o', yerr=zstds[ndist],label='Om_m')
        plt.errorbar(ndist-dx, Omp['y'], fmt='.', yerr=Omp['yerr'], label='Om_p')
        plt.errorbar(ndist-dx, fiducial_measurements['y'], fmt='o', yerr=fiducial_measurements['yerr'],label='fid')
        plt.xticks(ndist, labels)
        plt.xlabel(r"Distance Scale Range")
        plt.ylabel("a(z=0.5)")
        plt.title("Quijote Varying Distance Scale")
        plt.legend()
        plt.savefig(f"DirectionalParamScan_scalez_Om_m_{dx:.2f}_100_1e7.png",dpi=230)

        plt.figure(figsize=(10,8))
        #dx = 0.05
        plt.errorbar(ndist+dx, zmeans[ndist], fmt='.', yerr=zstds[ndist],label='Om_m')
        plt.errorbar(ndist-dx, Omp['y'], fmt='.', yerr=Omp['yerr'], label='Om_p')
        plt.errorbar(ndist, fiducial_measurements['y'], fmt='.', yerr=fiducial_measurements['yerr'],label='fid')
        plt.xticks(ndist, labels)
        plt.xlabel(r"Distance Scale Range")
        plt.ylabel("a(z=0.5)")
        plt.title("Quijote Varying Distance Scale")
        plt.legend()
        plt.savefig(f"DirectionalParamScan_scalez_Om_m_{dx:.2f}_px_100_1e7.png",dpi=230)
    ...
    yerr = zstds[ndist]
    labels = labels
    np.savez('QPS_Omm_100_1e7',x=x,y=y,yerr=yerr,labels=labels)#, nzstds=nzstds[ndist])
