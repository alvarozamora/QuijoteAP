...
if yt.is_root():
    ...
    for dx in np.arange(0.00,0.101,0.01):
        plt.figure(figsize=(10,8))
        #dx = 0.05
        plt.errorbar(ndist+dx, zmeans[ndist], fmt='o', yerr=zstds[ndist],label='Om_p')
        ...
        plt.title("Quijote Varying Distance Scale")
        plt.legend()
        plt.savefig(f"DirectionalParamScan_scalez_Om_p_{dx:.2f}_100_1e7.png",dpi=230)

        plt.figure(figsize=(10,8))
        #dx = 0.05
        plt.errorbar(ndist+dx, zmeans[ndist], fmt='.', yerr=zstds[ndist],label='Om_p')
        plt.errorbar(ndist-dx, fiducial_measurements['y'], fmt='.', yerr=fiducial_measurements['yerr'],label='fid')
        plt.xticks(ndist, labels)
        plt.xlabel(r"Distance Scale Range")
        plt.ylabel("a(z=0.5)")
        plt.title("Quijote Varying Distance Scale")
    ...
    yerr = zstds[ndist]
    labels = labels
    np.savez('QPS_Omp_100_1e7',x=x,y=y,yerr=yerr,labels=labels)#, nzstds=nzstds[ndist])
