...
#avg = CDFs.mean(0)
#zavg = zCDFs.mean(0)
directory = 'results_2_dilute'
avg, zavg = extract_results(directory)#results)
import pdb; pdb.set_trace()
...
plt.tight_layout()
plt.savefig('cdf_results_dilute.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_ratio_results_dilute.png')
'''
...
plt.tight_layout()
plt.savefig('zcdf_results_dilute.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('zcdf_ratio_results_dilute.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].set_ylim(1e-3,1)
plt.tight_layout()
plt.savefig('bothcdf_results_dilute.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('zcdf_cdf_ratio_results_dilute.png')
