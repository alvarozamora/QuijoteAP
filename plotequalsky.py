...
done = glob.glob('results_2_equalsky/*.npz');

squarek = 4

#xcdfs  = np.zeros((len(done), 6, 2, 199, squarek))
#zcdfs = np.zeros((len(done), 6, 2, 199, squarek))
#zxcdfs  = np.zeros((len(done), 6, 2, 199, squarek))
#zzcdfs = np.zeros((len(done), 6, 2, 199, squarek))
...
print(len(x), len(z), len(zx), len(zz), len(done), failed, len(done)-failed)
...
plt.tight_layout()
plt.savefig('cdf_results_equalsky.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xx_ratio.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_zz_ratio.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xz_ratio_realspace.png')

print("Plotting CDF-yratios in real space")
fig, ax = plt.subplots(len(k), 1, figsize=(20,10*len(k)))
for kk, knn in enumerate(k):
    for q, s in enumerate(S):
        y_pcdf = p_avg[q,1,:, kk]
        ...
        thecdf = np.logspace(-3, np.log10(0.5), 100)
        thecdf = np.append(thecdf, 1-thecdf[::-1][1:])

        denom = np.interp(p_avg[q,0,:, kk], p_avg[0,0,:,kk], thecdf)

        ax[kk].semilogx(x_pcdf, y_pcdf/denom, '.-', label=f'{s:.2f}')
    ax[kk].set_xlabel(r'Distance (Mpc $h^{-1}$)')
    ax[kk].set_ylabel(f'{knn}NN Peaked CDF Ratio')
    ax[kk].set_title(f'{knn}NN Peaked CDF Ratio (p/z)')
    ax[kk].grid(alpha=0.3)
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xx_yratio_realspace_renorm.png')

print("Plotting CDFx-ratios in real space")
fig, ax = plt.subplots(len(k), 1, figsize=(20,10*len(k)))
for kk, knn in enumerate(k):
    for q, s in enumerate(S):
        y_pcdf = p_avg[q,1,:,kk]
        ...
        #thecdf = np.logspace(-3, np.log10(0.5), 100)
        #thecdf = np.append(p, 1-p[::-1][1:])

        #denom = np.interp(p_avg[q,1,:, kk], p_avg[0,1,:,kk], p_avg[0,0,:, kk])

        ax[kk].semilogx(x_pcdf, x_pcdf/p_avg[0,0,:,kk], '.-', label=f'{s:.2f}')
    ax[kk].set_xlabel(r'Distance (Mpc $h^{-1}$)')
    ax[kk].set_ylabel(f'{knn}NN Peaked CDF Ratio')
    ax[kk].set_title(f'{knn}NN Peaked CDF Ratio (p/z)')
    ax[kk].grid(alpha=0.3)
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xx_xratio_realspace_renorm.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xz_ratio_redshiftspace.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xz_ratio_redshiftspace_renorm.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xz_ratio_redshiftspace_renormzoom.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xz_ratio_redshiftspace_fakerenorm.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_equalsky_xz_ratio_redshiftspace_fakerenormzoom.png')
