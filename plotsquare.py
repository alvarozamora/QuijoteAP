...
done = glob.glob('results_2_square/*.npz');

squarek = 4

xcdfs  = np.zeros((len(done), 4, 2, 199, squarek))
zcdfs = np.zeros((len(done), 4, 2, 199, squarek))
zxcdfs  = np.zeros((len(done), 4, 2, 199, squarek))
zzcdfs = np.zeros((len(done), 4, 2, 199, squarek))
...
plt.tight_layout()
plt.savefig('cdf_results_square.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_square_xx_ratio.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_square_zz_ratio.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_square_xz_ratio_realspace.png')
...
for kk, knn in enumerate(k):
    ...
    ax[kk].set_xlabel(r'Distance (Mpc $h^{-1}$)')
    ax[kk].set_ylabel(f'{knn}NN Peaked CDF Ratio')
    ax[kk].set_title(f'{knn}NN Peaked CDF Ratio (zp/zz)')
    ax[kk].grid(alpha=0.3)
    ax[kk].legend()
plt.tight_layout()
plt.savefig('cdf_results_square_xz_ratio_redshiftspace.png')
